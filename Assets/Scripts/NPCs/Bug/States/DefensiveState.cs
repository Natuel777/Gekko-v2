...
public class DefensiveState //: IState
{
    ...
    public DefensiveState(Bug bug) {_bug = bug;}

    public void Enter()
    {
        _timer = 0f;
        _bug.defensiveBehaviour.StartDefense(_bug.PlayerTransform);
    }

    public void Exit()
    {
        _bug.SetBugAttacking(false);
        _bug.defensiveBehaviour.StopDefense();
    }

    public void Update() 
    {
        _timer += Time.deltaTime;
        _bug.defensiveBehaviour.ArtificialUpdate();
